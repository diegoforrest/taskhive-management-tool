...
export function TopBar() {
  ...
  const getUserDisplayName = () => {
    ...
    if (user.email) {
      return user.email.value.split('@')[0]
    }
  ...
  const performSearch = React.useCallback(async (query: string) => {
    if (!query.trim() || !isAuthenticated || !user?.id) {
      setSearchResults([])
      return
    }
    ...
    setIsSearching(true)
    try {
      // Use projects from the useProjects hook
      const q = query.toLowerCase();

      // Only match project name or due date (ISO or formatted)
      const augmented = (projects || []).map((project) => {
        const displayName = project.name || `Project ${project.id.value}`;
                ...
                <DropdownMenuLabel className="font-normal">
                  <div className="flex flex-col space-y-1">
                    ...
                    <p className="text-xs leading-none text-muted-foreground truncate">
                      {isAuthenticated ? (user?.email?.value || 'No email') : 'Not signed in'}
                    </p>
                  </div>
                </DropdownMenuLabel>
                {isAuthenticated ? (
