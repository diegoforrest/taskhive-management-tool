...
export default function CompletedPage() {
			...
			try {
				...
				try {
					...
					try {
						...
						const arr = Array.isArray(projectsRes) ? projectsRes : (projectsRes && 'data' in (projectsRes as any) ? (projectsRes as any).data : []);
						const found = (arr as Project[]).find((p) => p.id === projectId) || null;
				...
				try {
					...
					const tArr = Array.isArray(tRes) ? tRes : (tRes && 'data' in (tRes as any) ? (tRes as any).data : []);
					if (mounted) setTasks(tArr as Task[]);
				} catch (e) {
				...
				try {
					...
					const hArr = Array.isArray(hRes) ? hRes : (hRes && 'data' in (hRes as any) ? (hRes as any).data : []);
					if (mounted) setProjectHistory(hArr as ChangeLogEntry[]);
				} catch (e) {
	...
	const openTaskHistory = async (task: Task) => {
		try {
			...
			const arr = Array.isArray(res) ? res : (res && 'data' in (res as any) ? (res as any).data : []);
				...
				<div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 lg:gap-0">
					<div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 lg:gap-6">
						...
						<div className="flex items-center gap-3 text-sm">
							...
							<div className="flex items-center gap-2">
								<span className={`text-xs font-semibold px-2 py-1 rounded-full ${project?.priority === 'High' ? 'bg-red-100 text-red-700' : project?.priority === 'Medium' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-700'}`}>
									Priority: {project?.priority || 'Medium'}
								</span>

								<Badge className="bg-green-100 text-green-800">{project?.status || 'Completed'}</Badge>
							</div>
						</div>
					</div>
																					...
																					{t.assignee && (
																						<div className="flex items-center gap-2">
																							<Avatar className="h-8 w-8">
																								<AvatarFallback>{getInitials(t.assignee)}</AvatarFallback>
																							</Avatar>
						...
						<Card>
							<CardContent>
								...
								{projectHistory.length === 0 ? (
									<div className="text-muted-foreground">No feedback found for this hive.</div>
								) : (
