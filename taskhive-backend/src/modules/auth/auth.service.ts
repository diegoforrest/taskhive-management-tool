...
export class AuthService {
	constructor(
		...
		@InjectRepository(require('./entities/refresh-token.entity').RefreshToken)
	...
	async login(loginDto: LoginDto) {
		...
		// Generate JWT token
		// Resolve roles from user.roles (stored as JSON string) if present
		let roles: string[] = [];
		try {
			if (user && (user as any).roles) {
				const raw = (user as any).roles;
				roles = Array.isArray(raw) ? raw : JSON.parse(raw);
			}
		} catch (e) {
			roles = [];
		}
		...
		const saltRounds = 12;
		const refreshHash = await require('bcryptjs').hash(rawRefresh, saltRounds);
	...
	// Exchange a raw refresh token for a new access token (and optionally rotate refresh)
	async refreshToken(rawRefresh: string) {
		try {
			...
			for (const t of tokens) {
				...
				const ok = await require('bcryptjs').compare(rawRefresh, t.token_hash);
				if (ok) {
					matched = t;
					break;
				}
			...
			let roles: string[] = [];
			try { if (user && (user as any).roles) { const raw = (user as any).roles; roles = Array.isArray(raw) ? raw : JSON.parse(raw); } } catch { roles = []; }
			const payload: JwtPayload = { sub: user.user_id, user_id: user.user_id, email: user.email, roles };
			const access_token = this.jwtService.sign(payload);
	...
	async createProject(user_id: number, name: string, description: string, due_date?: string | null, priority?: string) {
		try {
	...
	async updateProject(projectId: number, updateData: Partial<{ name: string; description: string; priority: string; due_date: string | null; status: string }>) {
		try {
